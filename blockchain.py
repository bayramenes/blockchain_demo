...
class BlockChain:
    ...
    # there is a room for improvement with utxo like bitcoin but i wont be doin it
    ...
    def verifyTx(self, publicKey, signature, message, sender, amount):
        ...
        if Ecdsa.verify(message=message, signature=sign, publicKey=public):
            if hashlib.sha256(publicKey.encode()).hexdigest().upper() == sender.upper():
                ...
                return "address no applicable"
        else:
            return "signature or message is wrong couldnt approve identity"
    ...
    def mineBlock(self):
        ...
        self.addTx(
            {"sender": "network", "receiver": self.nodeAddress, "amount": 1000}, index=0
        )
        block = {
